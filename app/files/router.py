...
@router.post("/")
async def post_file(data_input: FileInput = Body(), auth: str = Header()) -> dict[str, str]:
    user = await auth_check(auth)
...
@router.post("/merge")
async def merge_files(input_data: MergeInput = Body(), auth: str = Header()) -> dict[str, str]:
    user = await auth_check(auth)
...
async def post_file_by_id(
    file_id: str, auth: str = Header(), input_file: UploadFile = File()
) -> dict[str, str]:
    user = await auth_check(auth)
...
@router.get("/{file_id}")
async def get_file_by_id(file_id: str, auth: str = Header()) -> FileResponse:
    user = await auth_check(auth)
    ...
    return FileResponse(path=file.path, filename=file.filename, media_type="application/pdf")


@router.delete("/{file_id}")
async def delete_file_by_id(file_id: str, auth: str = Header()) -> dict[str, str]:
    user = await auth_check(auth)
